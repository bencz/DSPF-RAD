...
//   - log every dragstart so the source firing is verifiable
...
export class Palette {
    ...
    _onClick (ev) {
        ...
        if (!el) return;
        if (this._armedEl === el) { this.clearArmed(); return; }
        this.clearArmed();
        this._armedSpec = readSpec(el);
        this._armedEl   = el;
    ...
    _onDragStart (ev) {
        ...
        // Custom drag image so we don't snapshot the whole pill.
        attachDragGhost(ev, spec);
    }
}

function attachDragGhost (ev, spec) {
    try {
...
// Reads the dataset off a .palette-item into a spec the designer can act
// on.  Mirrors what readDropSpec rebuilds on the drop side.
...
// our palette.  Tries the custom MIME first, falls back to text/plain.
...
// Width in grid cells a freshly-dropped spec will occupy.  Used for the
// drop preview and the armed-cursor ghost.
