...
- 預設由 qualified source identity 產生；Inspector 新區可逐欄編輯。
- 集中管理 binding map：`{ sourceIdentity → runtimeBindingKey → domId }`。
- 生成 code 用該 id 綁定（如 `<TextField id="Z-XMG3tX" />` + `data-bind`）。
- id 跨 parse/adopt 穩定，不依賴會重生的 item id。
